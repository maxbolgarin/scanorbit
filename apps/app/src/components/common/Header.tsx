...
import { ChevronDown, LogOut, Orbit, Building2, Cloud, Plus, Check, Sun, Moon, Monitor } from "lucide-react";
...
export function Header() {
  ...
  const { user, org, authEnabled } = useAuthStore();
  const {
    currentAccountId,
    currentAccount,
    isOrgOverview,
  ...
  const handleLogout = () => {
    ...
    // Logout via same-origin proxy — the reverse proxy forwards to the API,
    // which clears the refresh_token cookie. GET navigation so the browser
    // sends the cookie (SameSite=Lax allows GET navigations).
