...
import { getMetrics, getContentType, dbPoolConnections, queueLength, usersTotal, orgsWithAwsAccounts } from './lib/metrics.js';
...
// Audit logging for all API requests (who did what, when)
app.use('/*', auditLog);
...
app.get('/metrics', async (c) => {
  try {
    ...
    const [userCountResult, orgsWithAwsResult] = await Promise.all([
