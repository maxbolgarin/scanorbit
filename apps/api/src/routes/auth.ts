...
authRoute.post('/refresh', async (c) => {
  // Single-user mode: issue a token for the built-in admin. No cookie required.
  if (!config.authEnabled) {
    const { userId, orgId } = await ensureSystemUserAndOrg();
    const { accessToken } = await setAuthTokens(c, userId, orgId);
...
authRoute.get('/me', requireAuth, async (c) => {
  ...
  return c.json({ ...result, authEnabled: config.authEnabled });
});
