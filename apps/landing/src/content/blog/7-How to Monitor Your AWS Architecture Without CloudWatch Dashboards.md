...
[ScanOrbit](https://scanorbit.cloud) is what I built to solve this specific problem for myself. It connects through a read-only IAM role, scans all regions in one pass, and gives you a complete inventory alongside security and cost findings. No agents, no CloudWatch configuration, no scripts to maintain. It's EU-hosted in Amsterdam, which matters if GDPR data residency is a concern for you. It shows your resource count and finding severity so you can see the scope. I'm biased obviously, but I couldn't find anything else that did exactly this without requiring a platform-level commitment.
