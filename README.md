...
Agentless AWS infrastructure scanner — discover resources, detect security issues, and ensure compliance.

Open source under **Apache 2.0**. Run it as a managed SaaS or self-host it on
your own server — every organization is on the full TEAM tier by default, no
billing required.

## Table of Contents

- [Self-host (Quick start)](#self-host-quick-start)
- [Self-host on a server (Terraform + Ansible)](#self-host-on-a-server-terraform--ansible)
- [Overview](#overview)
...
You need Docker and ~2 vCPU / 2 GB RAM. The bundled
`docker-compose.selfhost.yml` brings up the full stack (Postgres, Redis,
migrations, API, web app, scanner, analyzer) on a single host.
...
make self-host-up
# equivalent to: docker compose -f docker-compose.selfhost.yml up -d --build
```

The web UI is now on **http://localhost:8080**. Sign up — the first user
becomes the org admin and the org runs with TEAM-tier limits (no billing
prompts).

Without Resend / SMTP configured, the signup verification code is printed
to the API logs:

```bash
make self-host-logs            # or: docker compose -f docker-compose.selfhost.yml logs -f api
```

Behind a reverse proxy: set `FRONTEND_URL=https://scanorbit.example.com` in
`.env`, point Caddy/nginx/Traefik at the `app` container (it listens on plain
HTTP/80), and let it terminate TLS.

```bash
make self-host-down            # stop everything
```

## Self-host on a server (Terraform + Ansible)

For a hardened production deploy the repo ships Terraform modules and
Ansible playbooks that mirror what runs in the managed SaaS:

| Component | Path | Purpose |
|---|---|---|
| Terraform — bootstrap | [`terraform/bootstrap`](terraform/bootstrap) | one-time remote state bucket |
| Terraform — backup infra | [`terraform/layer1-backup`](terraform/layer1-backup) | DNS + object storage for backups |
| Terraform — servers | [`terraform/layer2-server`](terraform/layer2-server) | app + CI VMs, security groups, cloud-init |
| Ansible | [`ansible/`](ansible) | provision secrets, sync configs, restart services |
| Production compose stack | [`deploy/docker-compose.yml`](deploy/docker-compose.yml) | api/app/landing + Caddy + Prometheus + Grafana + Loki + backups |

High-level flow:

```bash
# 1. Provision the VMs (Scaleway by default — see terraform/layer2-server for variables)
cd terraform/layer2-server
terraform init && terraform apply

# 2. Bootstrap the app host: secrets, TLS, docker login (one-time)
cd ../../ansible
ansible-playbook playbooks/deploy.yml --tags bootstrap

# 3. Sync deploy/ configs and start the full production stack
ansible-playbook playbooks/sync.yml
ansible-playbook playbooks/deploy.yml --tags services
```

The full production guide (including DNS, GHCR + Watchtower auto-updates,
TLS, and monitoring tunnels) is in
[Production Deployment](#production-deployment) below. The compose stack at
`deploy/docker-compose.yml` includes the optional landing site, monitoring,
and backups; the simpler root-level `docker-compose.selfhost.yml` is what
you want for a single-tenant OSS install without those extras.
