...
## 5. Astro Implementation Tips

- Use **Astro islands** for React components (interactive sections).
- **Tailwind CSS** for rapid styling with your palette.
- **Heroicons** or **Lucide** for icons.
- **Lottie** or **Framer Motion** for scan animations.
- **shadcn/ui** components for consistency (if using React islands).

**Example Hero HTML** (Astro):
```astro
<section class="min-h-screen bg-gradient-to-br from-[#1a1f3a] to-[#0a0f1e] relative overflow-hidden">
  <!-- Animated scan overlay -->
  <div class="absolute inset-0 bg-[url('/scan-lines.svg')] opacity-20 animate-scan"></div>
  <div class="container mx-auto px-6 py-32 text-center relative z-10">
    <h1 class="text-6xl md:text-7xl font-bold bg-gradient-to-r from-purple-500 to-cyan-400 bg-clip-text text-transparent mb-6">
      ScanOrbit
    </h1>
