...
// walks up the parent chain (up to 6 levels) until it finds one. Already-set
// environment variables take precedence — `.env` only fills in what's missing.
// Safe to call from any cwd; silently no-ops if no file is found.
func LoadDotEnv() {
	dir, err := os.Getwd()
	if err != nil {
		return
